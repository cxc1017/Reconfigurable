module spi_rx(
	input clk,
	input rst,
	input sclk,
	input mosi,
	input ssn,
	input [15:0] read_data_fifo,
	output logic wrreq,
	output logic rdreq,
	output logic wrreq_neg,
	output logic rdreq_neg,
	output logic [7:0] address,
	output logic [15:0] data,
	output logic [15:0] read_data,
	output logic [15:0] sum_data,
	output logic read_en,
	output logic write_en,
	output logic tx_req
);
	
	logic shift_en, load_rw, load_addr, rx_finish, rst_data_cnt, load_data, chk_sum, read_chk_sum;
	logic s_signal, d_signal, s_signal1, d_signal1, ssn_negedge, sclk_posedge, rw, chk_conv;
	logic [31:0] receive_data_cnt;
	logic [15:0] shift_data, empty_cnt, conv;
	logic [15:0] write_data, write_data_neg;
	logic [15:0] reg_file [255:0];
	
	//ssn_negedge edge_dectector
	always_ff@(posedge clk or posedge rst)
	begin
		if(rst)
		begin
			s_signal <= 1'b0;
			d_signal <= 1'b0;
			ssn_negedge <= 1'b0;
		end
		else
		begin
			{d_signal, s_signal} <= {s_signal, ssn};
			ssn_negedge <= ~s_signal & d_signal;
		end
	end
	
	//sclk_posedge edge_dectector
	always_ff@(posedge clk or posedge rst)
	begin
		if(rst)
		begin
			s_signal1 <= 1'b0;
			d_signal1 <= 1'b0;
			sclk_posedge <= 1'b0;
		end
		else
		begin
			{d_signal1, s_signal1} <= {s_signal1, sclk};
			sclk_posedge <= s_signal1 & ~d_signal1;
		end
	end

	//neg
	always_ff@(negedge clk or posedge rst)
	begin
		if(rst)
		begin
			write_data_neg <= 0;
			wrreq_neg <= 0;
			rdreq_neg <= 0;
		end
		else 
		begin
			write_data_neg <= data;
			wrreq_neg <= wrreq;
			rdreq_neg <= rdreq;
		end
	end
	
	//cnt
	always_ff@(posedge clk or posedge rst)
	begin
		if(rst)
			receive_data_cnt <= 0;
		else if(rst_data_cnt)
			receive_data_cnt <= 0;
		else if(sclk_posedge)
			receive_data_cnt <= receive_data_cnt + 1;
	end
	
	//shift_register
	always_ff@(posedge clk or posedge rst)
	begin
		if(rst)
			shift_data <= 0;
		else if(ssn)
			shift_data <= 0;
		else if(shift_en)
			shift_data <= {shift_data[14:0], mosi};			
	end
	
	//rw register
	always_ff@(posedge clk or posedge rst)
	begin
		if(rst)
			rw <= 0;
		else if(load_rw)
			rw <= shift_data[0];
	end
	//address
	always_ff@(posedge clk or posedge rst)
	begin
		if(rst)
			address <= 0;
		else if(load_addr)
			address <= shift_data[7:0];
	end
	//data
	always_ff@(posedge clk or posedge rst)
	begin
		if(rst)
			data <= 0;
		else if(load_data)
			data <= shift_data;
	end

	//sum
	always_ff@(posedge clk or posedge rst)
	begin
		if(rst)
			sum_data <= 0;
		else if(chk_sum)
			sum_data <= sum_data + read_data_fifo;
	end
	//empty
	always_ff@(posedge clk or posedge rst)
	begin
		if(rst)
			empty_cnt <= 0;
		else if(wrreq)
			empty_cnt <= empty_cnt + 1;
		else if(chk_sum)
			empty_cnt <= empty_cnt - 1;
	end
	
	//register_file
	always_ff@(posedge clk)
	begin
		if(write_en)	//reg_file
			reg_file[address] <= data;
		else if(read_en)
			read_data <= reg_file[address];
	end
	
	typedef enum {INIT, START_SPI_RX, RECEIVE_ADDRESS, DUMMY, SUM
				, CHECK_COMMAND, RECEIVE_DATA, TX_REQ, FINISH, WRITE, RECEIVE_COMMAND}state_rx;
	state_rx ps, ns;
	
	always_ff@(posedge clk or posedge rst)
	begin
		if(rst)
			ps <= INIT;
		else 
			ps <= ns;
	end
	
	//FSM
	always_comb
	begin
		shift_en = 0; load_rw = 0; load_addr = 0; load_data = 0; rst_data_cnt = 0;
		ns = ps; read_en = 0; write_en = 0; tx_req = 0; rx_finish = 0; chk_sum = 0;
		rdreq = 0; wrreq = 0;
		case(ps)
			INIT:
			begin
				ns = START_SPI_RX;
			end
			START_SPI_RX:
			begin
				if(ssn_negedge)
				begin
					//shift_en = 1;
					rst_data_cnt = 1;
					ns = RECEIVE_ADDRESS;
				end
				else
					ns = START_SPI_RX;
			end
			RECEIVE_ADDRESS:
			begin
				if(receive_data_cnt > 7)	//傳完後載入address
				begin
					load_addr = 1;
					ns = RECEIVE_COMMAND;
				end
				else 				//0~7bit傳address
				begin
					if(sclk_posedge)
						shift_en = 1;
					ns = RECEIVE_ADDRESS;
				end
			end
			RECEIVE_COMMAND:
			begin
				if(receive_data_cnt == 9)		//第9bit載入
				begin
					load_rw = 1;
					ns = DUMMY;
				end
				else 							//第8bit傳rw
				begin
					if(sclk_posedge)
						shift_en = 1;
				end
			end
			DUMMY:
			begin
				if(receive_data_cnt > 15)	
				begin
					rst_data_cnt = 1;
					ns = CHECK_COMMAND;
				end
				else 					//9-15bit don't care
				begin
					ns = DUMMY;
				end
			end
			CHECK_COMMAND:
			begin
				if(rw)								//rw = 1 -> read
				begin
					if(address == 8'h81)
					begin
						chk_sum = 1;
						ns = SUM;
					end
					else if(address == 8'h82)
					begin
						//chk_sum = 1;
						rdreq = 1;
						ns = SUM;
					end
					else 
					begin
						read_en = 1;
						ns = SUM;
					end	
				end
				else 
				begin								//rw = 0 -> write
					rst_data_cnt = 1;				//16bit結束, 要重算
					ns = RECEIVE_DATA;
				end
			end
			SUM:
			begin
				if(empty_cnt > 0)
				begin
					chk_sum = 1;
					ns = CHECK_COMMAND;
				end
				else 
				begin
					ns = TX_REQ;
				end
			end
			TX_REQ:
			begin
				tx_req = 1;
				ns = FINISH;
			end
			RECEIVE_DATA:
			begin
				if(receive_data_cnt > 15)			//16bit傳完, 要載入
				begin
					load_data = 1;
					ns = WRITE;
				end
				else 
				begin						//0~15bit傳資料
					if(sclk_posedge)
						shift_en = 1;
					ns = RECEIVE_DATA;
				end
			end
			WRITE:
			begin
				if(address == 8'h80)
				begin
					wrreq = 1;
				end
				else
				begin
					write_en = 1;
				end
				ns = FINISH;
			end
			FINISH:
			begin
				rst_data_cnt = 1;
				rx_finish = 1;
				ns = INIT;
			end
		endcase
	end
	
endmodule